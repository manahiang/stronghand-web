import React, { useEffect, useState } from 'react'
import { Link,useParams} from 'react-router-dom'
import { Avatar } from 'primereact/avatar'
import "primereact/resources/themes/lara-light-indigo/theme.css";  //theme
import "presimereact/resources/primereact.min.css";                  //core css
import "primeicons/primeicons.css";
import "primereact/resources/them/bootstrap4-light-blue/theme.css";
import "primeflex/primeflex.min.css"
import { BsFillPlayCircleFill } from "react-icons/bs";
import Markdown from 'markdown-to-jsx';
import Footer from '../components/footer';
import data from "../language/blogDetail.json";
const fb = 'https://web.facebook.com/sh.homefix';
const telegram = 'https://t.me/sh.homefix';

function BlogDetail() {
    {
        "profile": {
            "image": "https://imageio.forbes.com/specials-images/imageserve/09kZdVWgJO0tu/960x960.jpg?format=jpg&width=960",
            "author": "Mark Zuckerberg",
            "date": "Jul22,2023",
            "id":1
        },
        "headline": {
            "title": "10 Concepts to Improve Your Mastery of JavaScript",
            "dec": "JavaScript is unarguably one of the most popular programming languages in the world and is used to create interactive websites and web applications. For developers, learning JavaScript can lead to a",
            "pic": "https://miro.medium.com/fit/c/168/168/1*UhDmsFamDWhg33YQ_hwHhg.png",
            "markPic": "https://miro.medium.com/v2/resize:fit:786/0*sk-gGCWt8ZrGY9z4"
        }
    }
    const file = ["title.md", "des.md", "footer.md", "header.md", "body.md"]
    const [post, setPost] = useState("");
    useEffect(() => {
        import(`../markdown/${file[1]}`)
            .then(res => {
                fetch(res.default)
                    .then(response => response.text())
                    .then(response => setPost(response))
            })
    })

    return (
        <>
            <div style={{ backgroundColor: 'white' }}>

                {
                    data.data.map((item)=>{
                        return(

                            <div className='blog_detail_container text-color pt-8' style={{ maxWidth: '1200px', height: 'auto', margin: 'auto' }}>
                        <div className='flex align-items-center'>
                            <Avatar image={item.profile.image}
                                size='xlarge' shape='circle' className='m-2' />
                            <div className='line-height-1 bill'>
                                <p className='title text-xl name1'>{item.profile.author}</p>
                                <div className='flex align-items-center name2'>
                                    <p className='date'>{item.profile.date}</p>
                                    <p className='ml-2 point'>-</p>
                                    <p className='ml-2 mn1'>9mn read</p>
                                    <p className='ml-2'>-</p>
                                    <i className='pi pi-star ml-2 -mt-1 star' style={{ color: '#F2921D' }}></i>
                                    <p className='ml-2 member1'>Member-only</p>
                                    <Link to=""><BsFillPlayCircleFill className='text-white ml-4' /></Link>
                                    <Link to="" className='no-underline text white'><p className='text-white ml-1'>Listen</p></Link>
                                </div>
                            </div>
                            <a href={fb} className='text-color'><i className='pi pi-facebook ml-8 text-2xl m-2'></i></a>
                            <a href={telegram} className='text-color'><i className='pi pi-telegram ml-3 text-2xl m-2'></i></a>

                        </div>
                        <h1 className='m-2 mt-4 font-bold'>9 Awesome JavaScript Projects for your Resume</h1>
                        <h2 className='m-2 mt-4 font-normal'>Build projects & get a high-paying developer job</h2>
                        <div className="m-3 mt-8">
                            <img src={item.headline.markPic} alt='' className='w-full image border-round-md' />

                            {/* markdown file */}

                            <div className='m-2 mt-8 text-color '>
                                <Markdown className="text-2xl line-height-3 markdown" style={{ paddingBottom: '25px' }}>
                                    {post}
                                </Markdown>
                            </div>
                        </div>

                    </div>
                        )
                    })
                }
            </div>
            <div className='mt-4'>
                <Footer />
            </div>
        </>
    )
}

export default BlogDetail