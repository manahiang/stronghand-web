            ...
            <div style={{ backgroundColor: 'white', paddingTop: '5px' }}>
                <div style={{ maxWidth: '1200px', height: 'auto', margin: 'auto' }}>
                    {data.data.map((item, index) =>
                        <>
                            {
                                console.log("this is it", item[_lan].profile.image)
                            }
                            <div className='flex align-items-center justify-content-between mt-8 text-color'>
                                <div className='m-2'>
                                    <div className='flex align-items-center'>
                                        <Avatar image={item[_lan].profile.image}
                                            size='large' shape='circle' className='m-2' />
                                        <p className='name'>{item[_lan].en.profile.author}</p>
                                        <p className='name ml-2'>{item[_lan].en.profile.date}</p>
                                    ...
                                    <Link to={`blog-detail/${index}`} className='no-underline text-color'>
                                        <h1 className='title '>{item[_lan].en.headline.title}</h1>
                                ...
                                <img src={item[_lan].en.headline.pic} className='m-2 img' alt='pop' />
                            </div><hr />
                        </>
                    )}
                </div>
