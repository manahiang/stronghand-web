...
In *concurrentPromise*, *Promise.all* wires up the promise chain in one go, meaning that the operationwill fail-fast regaredless fo the order of rejection of the promises, and the error will always occur withi the configured proise chain, enabling it to be a
