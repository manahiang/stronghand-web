...
Code after each await expression can be thought fo as existing in _.then_ callback. In this way a promise chain is progressiely constasync function foo() {
  await 1;
} ructed with each reentrant step through the function. The return value forms the fincal link in the chain.
